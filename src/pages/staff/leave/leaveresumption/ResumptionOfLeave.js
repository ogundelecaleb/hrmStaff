...
const ResumptionOfLeave = () => {
  ...
  async function getResumptionConfirmation(page) {
    const response = await api.fetchLeaveResumptionRequest({ params: { page } })
    return response;
  }

  const { isLoading, isError, data, error, isPreviousData, refetch } = useQuery(['resumptionRequests', page], () =>
    getResumptionConfirmation(page),
    {

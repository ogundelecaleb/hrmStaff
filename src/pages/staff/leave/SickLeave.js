...
const SickLeave = ({ navigate }) => {
  ...
  console.log(fullName,maritalStatus,departmentOrUnitOrFacultyID,dateOfFirstAppointment,rankDesignation,selectedLeaveType,startDate,endDate,addressLeave, resumptionDate,leaveDuration);
  ...
  const calculateLeaveDuration = () => {
    if (startDate && endDate) {
      ...
      const maxDays = leaveReason === 'yes' ? 183 : 42;

      if (durationInDays > maxDays) {
        enqueueSnackbar('Leave duration cannot exceed ' + maxDays + ' days (Mon-Fri).', { variant: 'error' });
        setEndDate('');
        setResumptiondate('')
      } else {
            ...
            <div class='pb-5' style={{ display: !showAdditionalInfo ? "block" : "none" }}>
              ...
              <div class='mb-3'>
                <label
                  ...
                  Are you currently hospitalized<sup className='text-danger'>*</sup>
                </label>
                <select
              ...
              <div class='mb-3'>
                ...
                <input class='form-control rounded-0' value={staffRep} onChange={(e) => setStaffrep(e.target.value)}/>
              </div>
              <button
                type='button'
